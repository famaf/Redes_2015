...
HTTP_PORT = 80   # El puerto por convencion para HTTP,
                 # segÃºn http://tools.ietf.org/html/rfc1700
...
def parse_server(url):
    ...
    El llamador es el dueÃ±o de la memoria devuelta
...
def connect_to_server(server_name):
    ...
    ip_address = socket.gethostbyname(server_name)  # OBTENGO DIRRECCION IP
...
def check_http_response(header):
    ...
    Ã©xito. Un encabezado de respuesta HTTP tiene la forma
...
def get_response(connection, filename):
    ...
    Devuelve True en caso de Ã©xito, False en caso contrario
    """
    BUFFER_SIZE = 4096
...
def download(url, filename):
    ...
    try:
        ...
        # Descomentar la siguiente lÃ­nea para debugging:
        # raise
        sys.exit(1)
